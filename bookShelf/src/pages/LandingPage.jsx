  ...
  return (
    ...
    <div className={`${containerClasses} ${fullWidthContainerClasses} md:flex-row justify-between`}>
        ...
        <div className={halfWidthClasses}>
            ...
            <span className='text-3xl font-bold  text-yellow-500 relative z-20 drop-shadow-2xl'>Bookshelf</span>
            </div>
            ...
            <button className="bg-yellow-300 text-white py-2 px-4 rounded mt-4 font-bold hover:ml-4 transition-all duration-300"
             onClick={() => {navigate("/login")}}>Let's Read</button>

        </div>
        <div className={`${halfWidthClasses} mt-6 md:mt-0 ${imageContainerClasses}`}>
        <div className="absolute top-0 left-0 w-24 h-24 bg-yellow-500"></div>
            <img src={landingImg} alt="Person walking through door" className="w-full h-full" />
            <div className="absolute bottom-0 right-0 w-24 h-24 bg-yellow-500"></div>
        </div>
    </div>
</div>
  )
